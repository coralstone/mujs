...
static void jb_load(js_State *J)
{
...
static void jb_print(js_State *J)
{
...
static void jb_write(js_State *J)
{
	int i, top = js_gettop(J);
	for (i = 1; i < top; ++i) {
		const char *s = js_tostring(J, i);
...
static void jb_read(js_State *J)
{
	const char *filename = js_tostring(J, 1);
	FILE *f;
	char *s;
...
static void jb_readline(js_State *J)
{
	char line[256];
	int n;
	...
	if (n > 0 && line[n-1] == '\n')
		line[n-1] = 0;
	js_push_string(J, line);
}

static void jb_quit(js_State *J)
{
