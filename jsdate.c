...
#define js_optnumber(J,I,V) (js_is_def(J,I) ? js_tonumber(J,I) : V)
...
{
	js_push_number(J, Now());
}

static void D_date(js_State *J)
{
	char buf[64];
	js_push_string(J, fmtdatetime(buf, LocalTime(Now()), LocalTZA()));
}

static void D_new_date(js_State *J)
{
	int top = js_gettop(J);
	js_Object *obj;
	double t;
...
static void Dp_toString(js_State *J)
{
	char buf[64];
	double t = js_todate(J, 0);
	js_push_string(J, fmtdatetime(buf, LocalTime(t), LocalTZA()));
}
...
static void Dp_toDateString(js_State *J)
{
	char buf[64];
	double t = js_todate(J, 0);
	js_push_string(J, fmtdate(buf, LocalTime(t)));
}
...
static void Dp_toTimeString(js_State *J)
{
	char buf[64];
	double t = js_todate(J, 0);
	js_push_string(J, fmttime(buf, LocalTime(t), LocalTZA()));
}
...
static void Dp_toUTCString(js_State *J)
{
	char buf[64];
	double t = js_todate(J, 0);
	js_push_string(J, fmtdatetime(buf, t, 0));
}
...
{
	double t = js_todate(J, 0);
	js_push_number(J, YearFromTime(LocalTime(t)));
}

static void Dp_getMonth(js_State *J)
{
	double t = js_todate(J, 0);
	js_push_number(J, MonthFromTime(LocalTime(t)));
}

static void Dp_getDate(js_State *J)
{
	double t = js_todate(J, 0);
	js_push_number(J, DateFromTime(LocalTime(t)));
}

static void Dp_getDay(js_State *J)
{
	double t = js_todate(J, 0);
	js_push_number(J, WeekDay(LocalTime(t)));
}

static void Dp_getHours(js_State *J)
{
	double t = js_todate(J, 0);
	js_push_number(J, HourFromTime(LocalTime(t)));
}

static void Dp_getMinutes(js_State *J)
{
	double t = js_todate(J, 0);
	js_push_number(J, MinFromTime(LocalTime(t)));
}

static void Dp_getSeconds(js_State *J)
{
	double t = js_todate(J, 0);
	js_push_number(J, SecFromTime(LocalTime(t)));
}

static void Dp_getMilliseconds(js_State *J)
{
	double t = js_todate(J, 0);
	js_push_number(J, msFromTime(LocalTime(t)));
}

static void Dp_getUTCFullYear(js_State *J)
{
	double t = js_todate(J, 0);
	js_push_number(J, YearFromTime(t));
}

static void Dp_getUTCMonth(js_State *J)
{
	double t = js_todate(J, 0);
	js_push_number(J, MonthFromTime(t));
}

static void Dp_getUTCDate(js_State *J)
{
	double t = js_todate(J, 0);
	js_push_number(J, DateFromTime(t));
}

static void Dp_getUTCDay(js_State *J)
{
	double t = js_todate(J, 0);
	js_push_number(J, WeekDay(t));
}

static void Dp_getUTCHours(js_State *J)
{
	double t = js_todate(J, 0);
	js_push_number(J, HourFromTime(t));
}

static void Dp_getUTCMinutes(js_State *J)
{
	double t = js_todate(J, 0);
	js_push_number(J, MinFromTime(t));
}

static void Dp_getUTCSeconds(js_State *J)
{
	double t = js_todate(J, 0);
	js_push_number(J, SecFromTime(t));
}

static void Dp_getUTCMilliseconds(js_State *J)
{
	double t = js_todate(J, 0);
	js_push_number(J, msFromTime(t));
}

static void Dp_getTimezoneOffset(js_State *J)
{
	double t = js_todate(J, 0);
	js_push_number(J, (t - LocalTime(t)) / msPerMinute);
}
...
static void Dp_toJSON(js_State *J)
{
	...
	if (js_is_number(J, -1) && !isfinite(js_tonumber(J, -1))) {
		js_push_null(J);
		return;
	}
	js_pop(J, 1);

	js_get_prop(J, 0, "toISOString");
	if (!js_is_callable(J, -1))
		js_error_type(J, "Date.prototype.toJSON: this.toISOString not a function");
	js_copy(J, 0);
	js_call(J, 0);
}
...
void jb_initdate(js_State *J)
{
	...
	js_push_object(J, J->Date_prototype);
	{
		jb_prop_func(J, "Date.prototype.valueOf", Dp_valueOf, 0);
		jb_prop_func(J, "Date.prototype.toString", Dp_toString, 0);
		jb_prop_func(J, "Date.prototype.toDateString", Dp_toDateString, 0);
		jb_prop_func(J, "Date.prototype.toTimeString", Dp_toTimeString, 0);
		jb_prop_func(J, "Date.prototype.toLocaleString", Dp_toString, 0);
		jb_prop_func(J, "Date.prototype.toLocaleDateString", Dp_toDateString, 0);
		jb_prop_func(J, "Date.prototype.toLocaleTimeString", Dp_toTimeString, 0);
		jb_prop_func(J, "Date.prototype.toUTCString", Dp_toUTCString, 0);

		jb_prop_func(J, "Date.prototype.getTime", Dp_valueOf, 0);
		jb_prop_func(J, "Date.prototype.getFullYear", Dp_getFullYear, 0);
		jb_prop_func(J, "Date.prototype.getUTCFullYear", Dp_getUTCFullYear, 0);
		jb_prop_func(J, "Date.prototype.getMonth", Dp_getMonth, 0);
		jb_prop_func(J, "Date.prototype.getUTCMonth", Dp_getUTCMonth, 0);
		jb_prop_func(J, "Date.prototype.getDate", Dp_getDate, 0);
		jb_prop_func(J, "Date.prototype.getUTCDate", Dp_getUTCDate, 0);
		jb_prop_func(J, "Date.prototype.getDay", Dp_getDay, 0);
		jb_prop_func(J, "Date.prototype.getUTCDay", Dp_getUTCDay, 0);
		jb_prop_func(J, "Date.prototype.getHours", Dp_getHours, 0);
		jb_prop_func(J, "Date.prototype.getUTCHours", Dp_getUTCHours, 0);
		jb_prop_func(J, "Date.prototype.getMinutes", Dp_getMinutes, 0);
		jb_prop_func(J, "Date.prototype.getUTCMinutes", Dp_getUTCMinutes, 0);
		jb_prop_func(J, "Date.prototype.getSeconds", Dp_getSeconds, 0);
		jb_prop_func(J, "Date.prototype.getUTCSeconds", Dp_getUTCSeconds, 0);
		jb_prop_func(J, "Date.prototype.getMilliseconds", Dp_getMilliseconds, 0);
		jb_prop_func(J, "Date.prototype.getUTCMilliseconds", Dp_getUTCMilliseconds, 0);
		jb_prop_func(J, "Date.prototype.getTimezoneOffset", Dp_getTimezoneOffset, 0);

		jb_prop_func(J, "Date.prototype.setTime", Dp_setTime, 1);
		jb_prop_func(J, "Date.prototype.setMilliseconds", Dp_setMilliseconds, 1);
		jb_prop_func(J, "Date.prototype.setUTCMilliseconds", Dp_setUTCMilliseconds, 1);
		jb_prop_func(J, "Date.prototype.setSeconds", Dp_setSeconds, 2);
		jb_prop_func(J, "Date.prototype.setUTCSeconds", Dp_setUTCSeconds, 2);
		jb_prop_func(J, "Date.prototype.setMinutes", Dp_setMinutes, 3);
		jb_prop_func(J, "Date.prototype.setUTCMinutes", Dp_setUTCMinutes, 3);
		jb_prop_func(J, "Date.prototype.setHours", Dp_setHours, 4);
		jb_prop_func(J, "Date.prototype.setUTCHours", Dp_setUTCHours, 4);
		jb_prop_func(J, "Date.prototype.setDate", Dp_setDate, 1);
		jb_prop_func(J, "Date.prototype.setUTCDate", Dp_setUTCDate, 1);
		jb_prop_func(J, "Date.prototype.setMonth", Dp_setMonth, 2);
		jb_prop_func(J, "Date.prototype.setUTCMonth", Dp_setUTCMonth, 2);
		jb_prop_func(J, "Date.prototype.setFullYear", Dp_setFullYear, 3);
		jb_prop_func(J, "Date.prototype.setUTCFullYear", Dp_setUTCFullYear, 3);

		/* ES5 */
		jb_prop_func(J, "Date.prototype.toISOString", Dp_toISOString, 0);
		jb_prop_func(J, "Date.prototype.toJSON", Dp_toJSON, 1);
	}

	js_new_cctor(J, D_date, D_new_date, "Date", 0); /* 1 */
	{
		jb_prop_func(J, "Date.parse", D_parse, 1);
		jb_prop_func(J, "Date.UTC", D_UTC, 7);
