...
JS_NORETURN void jc_error(js_State *J, js_Ast *node, const char *fmt, ...) JS_PRINTFLIKE(3,4);
...
void jc_error(js_State *J, js_Ast *node, const char *fmt, ...)
{
	va_list ap;
	char buf[512];
	char msgbuf[256];
...
static void checkfutureword(JF, js_Ast *exp)
{
	if (jsY_findword(exp->string, futurewords, nelem(futurewords)) >= 0)
		jc_error(J, exp, "'%s' is a future reserved word", exp->string);
	if (F->strict) {
		if (jsY_findword(exp->string, strictfuturewords, nelem(strictfuturewords)) >= 0)
			jc_error(J, exp, "'%s' is a strict mode future reserved word", exp->string);
	}
}
...
static void emitraw(JF, int value)
{
	if (value != (js_Instruction)value)
		js_error_syntax(J, "integer overflow in instruction coding");
...
static void addlocal(JF, js_Ast *ident, int reuse)
{
	...
	if (F->strict) {
		if (!strcmp(name, "arguments"))
			jc_error(J, ident, "redefining 'arguments' is not allowed in strict mode");
		if (!strcmp(name, "eval"))
			jc_error(J, ident, "redefining 'eval' is not allowed in strict mode");
	}
	if (reuse || F->strict) {
		int i;
		for (i = 0; i < F->varlen; ++i) {
			if (!strcmp(F->vartab[i], name)) {
				if (reuse)
					return;
				if (F->strict)
					jc_error(J, ident, "duplicate formal parameter '%s'", name);
			}
		}
	}
...
static void emitlocal(JF, int oploc, int opvar, js_Ast *ident)
{
	...
	if (F->strict && oploc == OP_SETLOCAL) {
		if (!strcmp(ident->string, "arguments"))
			jc_error(J, ident, "'arguments' is read-only in strict mode");
		if (!strcmp(ident->string, "eval"))
			jc_error(J, ident, "'eval' is read-only in strict mode");
	}
	if (F->lightweight) {
...
static void emitjumpto(JF, int opcode, int dest)
{
	...
	if (dest != (js_Instruction)dest)
		js_error_syntax(J, "jump address integer overflow");
	emitraw(J, F, dest);
}
	...
	while (list->a != end) {
		if (list->a->type == end->type) {
			...
			if (!strcmp(needle, straw))
				jc_error(J, list, "duplicate property '%s' in object literal", needle);
		}
		list = list->b;
	}
}
...
static void cobject(JF, js_Ast *list)
{
	...
	while (list) {
		...
		if (prop->type == AST_IDENTIFIER || prop->type == EXP_STRING)
			...
			jc_error(J, prop, "invalid property name in object initializer");
...
static void cassign(JF, js_Ast *exp)
{
	...
	case EXP_MEMBER:
		...
		break;
	default:
		jc_error(J, lhs, "invalid l-value in assignment");
	}
}
...
static void cassignforin(JF, js_Ast *stm)
{
	...
	if (stm->type == STM_FOR_IN_VAR) {
		if (lhs->b)
			jc_error(J, lhs->b, "more than one loop variable in for-in statement");
	...
	case EXP_MEMBER:
		...
		break;
	default:
		jc_error(J, lhs, "invalid l-value in for-in loop assignment");
	}
}
...
static void cdelete(JF, js_Ast *exp)
{
	...
	case EXP_IDENTIFIER:
		if (F->strict)
			jc_error(J, exp, "delete on an unqualified name is not allowed in strict mode");
	...
	case EXP_MEMBER:
		...
		break;
	default:
		jc_error(J, exp, "invalid l-value in delete expression");
	}
}
...
static void cexp(JF, js_Ast *exp)
{
	...
	default:
		jc_error(J, exp, "unknown expression: (%s)", jsP_aststring(exp->type));
	}
}
	...
	L1 = emitjump(J, F, OP_TRY);
	{
		...
		if (F->strict) {
			if (!strcmp(catchvar->string, "arguments"))
				jc_error(J, catchvar, "redefining 'arguments' is not allowed in strict mode");
			if (!strcmp(catchvar->string, "eval"))
				jc_error(J, catchvar, "redefining 'eval' is not allowed in strict mode");
		}
...
static void cswitch(JF, js_Ast *ref, js_Ast *head)
{
	...
	for (node = head; node; node = node->b) {
		...
		if (clause->type == STM_DEFAULT) {
			if (def)
				jc_error(J, clause, "more than one default label in switch");
			def = clause;
		} else {
			cexp(J, F, clause->a);
...
static void cstm(JF, js_Ast *stm)
{
	...
	case STM_BREAK:
		if (stm->a) {
			...
			if (!target)
				jc_error(J, stm, "break label '%s' not found", stm->a->string);
		} else {
			target = breaktarget(J, F, stm->parent, NULL);
			if (!target)
				jc_error(J, stm, "unlabelled break must be inside loop or switch");
		}
	...
	case STM_CONTINUE:
		if (stm->a) {
			...
			if (!target)
				jc_error(J, stm, "continue label '%s' not found", stm->a->string);
		} else {
			target = continuetarget(J, F, stm->parent, NULL);
			if (!target)
				jc_error(J, stm, "continue must be inside loop");
		}
	...
	case STM_RETURN:
		...
		if (!target)
			jc_error(J, stm, "return not in function");
	...
	case STM_WITH:
		if (F->strict)
			jc_error(J, stm->a, "'with' statements are not allowed in strict mode");
		cexp(J, F, stm->a);
		emit(J, F, OP_WITH);
		cstm(J, F, stm->b);
		emit(J, F, OP_ENDWITH);
	...
	if (node->type == EXP_IDENTIFIER) {
		if (!strcmp(node->string, "arguments")) {
			...
			if (!node->parent || node->parent->type != EXP_CALL || node->parent->a != node)
				js_error_eval(J, "%s:%d: invalid use of 'eval'", J->filename, node->line);
			F->lightweight = 0;
		}
	}
