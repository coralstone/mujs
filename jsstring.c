...
{
	if (!js_is_coercible(J, idx))
		js_error_type(J, "string function called on null or undefined");
...
{
	js_new_string(J, js_gettop(J) > 1 ? js_tostring(J, 1) : "");
}

static void jsB_String(js_State *J)
{
	js_push_string(J, js_gettop(J) > 1 ? js_tostring(J, 1) : "");
}

static void Sp_toString(js_State *J)
{
	js_Object *self = js_toobject(J, 0);
	if (self->type != JS_CSTRING) js_error_type(J, "not a string");
	js_push_literal(J, self->u.s.string);
}
...
static void Sp_lastIndexOf(js_State *J)
{
	const char *haystack = checkstring(J, 0);
	const char *needle = js_tostring(J, 1);
	int pos = js_is_def(J, 2) ? js_tointeger(J, 2) : (int)strlen(haystack);
	int len = strlen(needle);
	int k = 0, last = -1;
	Rune rune;
...
static void Sp_slice(js_State *J)
{
	...
	int e = js_is_def(J, 2) ? js_tointeger(J, 2) : len;
...
static void Sp_match(js_State *J)
{
	...
	if (js_is_regexp(J, 1))
		...
		js_new_regexp(J, js_tostring(J, 1), 0);
	...
	while (a <= e) {
		...
		js_push_lstr(J, b, c - b);
		js_set_index(J, -2, len++);
...
loop:
	...
	if (js_is_callable(J, 2)) {
		...
		for (x = 0; m.sub[x].sp; ++x) /* arg 0..x: substring and subexps that matched */
			js_push_lstr(J, m.sub[x].sp, m.sub[x].ep - m.sub[x].sp);
		js_push_number(J, s - source); /* arg x+2: offset within search string */
...
static void Sp_replace_string(js_State *J)
{
	...
	if (js_is_callable(J, 2)) {
		js_copy(J, 2);
		js_push_undef(J);
		js_push_lstr(J, s, n); /* arg 1: substring that matched */
		js_push_number(J, s - source); /* arg 2: offset within search string */
...
static void Sp_split_regexp(js_State *J)
{
	...
	text  = checkstring(J, 0);
	re    = js_toregexp(J, 1);
	limit = js_is_def(J, 2) ? js_tointeger(J, 2) : 1 << 30;
	...
	while (a < e) {
		...
		if (len == limit) return;
		js_push_lstr(J, p, b - p);
		js_set_index(J, -2, len++);

		for (k = 1; k < m.nsub; ++k) {
			if (len == limit) return;
			js_push_lstr(J, m.sub[k].sp, m.sub[k].ep - m.sub[k].sp);
			js_set_index(J, -2, len++);
		}
...
static void Sp_split_string(js_State *J)
{
	...
	int limit = js_is_def(J, 2) ? js_tointeger(J, 2) : 1 << 30;
	int i, n;
	...
	for (i = 0; str && i < limit; ++i) {
		...
		if (s) {
			js_push_lstr(J, str, s-str);
			js_set_index(J, -2, i);
			str = s + n;
		} else {
			js_push_string(J, str);
			js_set_index(J, -2, i);
			str = NULL;
		}
	}
...
void jb_initstring(js_State *J)
{
	...
	js_push_object(J, J->String_prototype);
	{
		jb_prop_func(J, "String.prototype.toString", Sp_toString, 0);
		jb_prop_func(J, "String.prototype.valueOf", Sp_valueOf, 0);
		jb_prop_func(J, "String.prototype.charAt", Sp_charAt, 1);
		jb_prop_func(J, "String.prototype.charCodeAt", Sp_charCodeAt, 1);
		jb_prop_func(J, "String.prototype.concat", Sp_concat, 0); /* 1 */
		jb_prop_func(J, "String.prototype.indexOf", Sp_indexOf, 1);
		jb_prop_func(J, "String.prototype.lastIndexOf", Sp_lastIndexOf, 1);
		jb_prop_func(J, "String.prototype.localeCompare", Sp_localeCompare, 1);
		jb_prop_func(J, "String.prototype.match", Sp_match, 1);
		jb_prop_func(J, "String.prototype.replace", Sp_replace, 2);
		jb_prop_func(J, "String.prototype.search", Sp_search, 1);
		jb_prop_func(J, "String.prototype.slice", Sp_slice, 2);
		jb_prop_func(J, "String.prototype.split", Sp_split, 2);
		jb_prop_func(J, "String.prototype.substring", Sp_substring, 2);
		jb_prop_func(J, "String.prototype.toLowerCase", Sp_toLowerCase, 0);
		jb_prop_func(J, "String.prototype.toLocaleLowerCase", Sp_toLowerCase, 0);
		jb_prop_func(J, "String.prototype.toUpperCase", Sp_toUpperCase, 0);
		jb_prop_func(J, "String.prototype.toLocaleUpperCase", Sp_toUpperCase, 0);

		/* ES5 */
		jb_prop_func(J, "String.prototype.trim", Sp_trim, 0);
	}
	js_new_cctor(J, jsB_String, jsB_new_String, "String", 0); /* 1 */
	{
